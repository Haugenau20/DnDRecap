...
const AdminPanel: React.FC<AdminPanelProps> = ({ onClose }) => {
  const { user, userProfile, getAllowedUsers, addAllowedUser, removeAllowedUser } = useFirebase();
  ...
  const handleRemoveAllowedUser = async (email: string) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to remove allowed user');
    }
  };
                    ...
                    <tr key={allowedUser.email}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <Typography variant="body-sm" color="secondary">
                          {new Date(allowedUser.addedAt).toLocaleDateString()}
                        </Typography>
                      </td>
